#!/usr/bin/env python3
"""
Script to generate throughput statistics (mean, 95%, 99%) for each run in 
BLOCK-PROMO-2025-08-31 or ZONED-PROMO-2025-08-30 directories.
Based on plot_throughput.py structure.
"""
...
def generate_map_files(logs_dir, split_output_dir):
    ...
    # For each metrics directory, find the best matching experiment config
    for metrics_dir in metrics_dirs:
        metrics_name = metrics_dir.name
        
        # We'll match based on the fact that there should be one metrics dir per experiment
        # and they should be in the same order or we can match by timestamp proximity
        ...
        if len(experiment_configs) == len(metrics_dirs):
            ...
            # For now, just use a basic assignment
...
def format_throughput(throughput, unit="B/s"):
    """Format throughput with appropriate unit."""
    if throughput >= 1024 * 1024 * 1024:  # >= 1 GiB/s
        return f"{throughput / (1024 * 1024 * 1024):.2f} GiB/s"
    elif throughput >= 1024 * 1024:  # >= 1 MiB/s
        return f"{throughput / (1024 * 1024):.2f} MiB/s"
    elif throughput >= 1024:  # >= 1 KiB/s
...
def compare_interfaces(all_stats):
    """Compare ZNS and Block interface performance and calculate percentage differences."""
    ...
    for config_key in zns_configs:
        if config_key in block_configs:
            ...
            # For throughput, we want positive to mean ZNS is better, so we negate
...
def print_interface_comparison(comparisons):
    """Print detailed interface comparison results."""
    if not comparisons:
        return
    
    print("=" * 100)
    print("INTERFACE COMPARISON: ZNS vs Block")
    print("=" * 100)
    ...
    for comp in comparisons:
        # Format percentage differences with color-coded indicators
    ...
    mean_trend = "better" if avg_mean_diff > 0 else "worse" if avg_mean_diff < 0 else "similar"
    p95_trend = "better" if avg_p95_diff > 0 else "worse" if avg_p95_diff < 0 else "similar"
    p99_trend = "better" if avg_p99_diff > 0 else "worse" if avg_p99_diff < 0 else "similar"
    
    print(f"Average Mean Throughput:  ZNS is {abs(avg_mean_diff):.1f}% {mean_trend} than Block")
    ...
    if chunk_512m:
        ...
        chunk_512m_trend = "better" if chunk_512m_mean_diff > 0 else "worse"
    ...
    if chunk_64k:
        ...
        chunk_64k_trend = "better" if chunk_64k_mean_diff > 0 else "worse"
...
def export_latex_table(all_stats, output_file="throughput_results.tex", metric_name="bytes_total"):
    ...
    for config_key in sorted(zns_stats.keys()):
        if config_key in block_stats:
            ...
            # For throughput: if ZNS > block, this will be negative (since block - zns < 0)
            # But we want positive to mean ZNS is better, so we'll handle the sign in display
    ...
    latex_content.append("Name & Mean & P99 \\\\")
    latex_content.append("\\hline")
    ...
    for config in matched_configs:
        ...
        # Format throughput values with units
        def format_throughput_latex(throughput):
            if throughput >= 1024 * 1024 * 1024:  # >= 1 GiB/s
                return f"{throughput / (1024 * 1024 * 1024):.2f} GiB/s"
            elif throughput >= 1024 * 1024:  # >= 1 MiB/s
                return f"{throughput / (1024 * 1024):.2f} MiB/s"
        ...
        # For throughput, negate the percentage since (block - zns)/block gives negative when ZNS is better
        zns_mean_str = f"{format_throughput_latex(zns['mean'])} ({-mean_imp:+.2f}\\%)"
        zns_p99_str = f"{format_throughput_latex(zns['p99'])} ({-p99_imp:+.2f}\\%)"
        
        block_mean_str = f"{format_throughput_latex(block['mean'])}"
        block_p99_str = f"{format_throughput_latex(block['p99'])}"
    ...
    latex_content.append(f"\\caption{{Throughput comparison results for {metric_name}}}")
    latex_content.append(f"\\label{{tab:throughput_{metric_name.replace('_', '_')}}}")
...
def process_single_directory(data_dir, metric_name="bytes_total", bucket_seconds=60):
    """Process a single data directory and return statistics."""
    ...
    for metrics_dir in sorted(metrics_dirs):
        ...
        if len(data_points) < 2:
            print(f"    Warning: Not enough data points ({len(data_points)})")
            continue
        ...
        if not throughputs:
            print(f"    Warning: No throughput data calculated")
            continue
        
        # Calculate statistics
        throughput_array = np.array(throughputs)
        mean_tp = np.mean(throughput_array)
        percentile_95 = np.percentile(throughput_array, 95)
        percentile_99 = np.percentile(throughput_array, 99)
        median_tp = np.median(throughput_array)
        std_tp = np.std(throughput_array)
        min_tp = np.min(throughput_array)
        max_tp = np.max(throughput_array)
        ...
        stats = {
            ...
            'data_points': len(throughput_array),
            'mean': mean_tp,
            'median': median_tp,
            'std': std_tp,
            'min': min_tp,
        ...
        # Print statistics
        print(f"    Data points: {len(throughput_array)}")
        print(f"    Mean:        {format_throughput(mean_tp)}")
        print(f"    Median:      {format_throughput(median_tp)}")
        print(f"    Std Dev:     {format_throughput(std_tp)}")
        print(f"    Min:         {format_throughput(min_tp)}")
        print(f"    Max:         {format_throughput(max_tp)}")
        print(f"    95th %ile:   {format_throughput(percentile_95)}")
        print(f"    99th %ile:   {format_throughput(percentile_99)}")
        print()
    
    return dir_stats


def generate_throughput_statistics(data_dirs, metric_name="bytes_total", bucket_seconds=60, compare_interfaces_flag=False, latex_output=None):
    """Generate throughput statistics for all runs in the specified data directories."""
    
    if isinstance(data_dirs, str):
        data_dirs = [data_dirs]
    
    print(f"Generating throughput statistics for {len(data_dirs)} directory(ies)")
    print(f"Directories: {', '.join(data_dirs)}")
    print(f"Metric: {metric_name}")
    print(f"Bucket size: {bucket_seconds} seconds")
    print("=" * 80)
    ...
    for data_dir in data_dirs:
        dir_stats = process_single_directory(data_dir, metric_name, bucket_seconds)
    ...
    if all_stats:
        ...
        for stats in all_stats:
            mean_str = format_throughput(stats['mean']).split()[0]
            p95_str = format_throughput(stats['p95']).split()[0]
            p99_str = format_throughput(stats['p99']).split()[0]
        ...
        if zns_stats:
            ...
            print(f"  Mean throughput:  {format_throughput(np.mean(zns_means))}")
            print(f"  Mean 95th %ile:   {format_throughput(np.mean(zns_p95s))}")
            print(f"  Mean 99th %ile:   {format_throughput(np.mean(zns_p99s))}")
            print()
        ...
        if block_stats:
            ...
            print(f"  Mean throughput:  {format_throughput(np.mean(block_means))}")
            print(f"  Mean 95th %ile:   {format_throughput(np.mean(block_p95s))}")
            print(f"  Mean 99th %ile:   {format_throughput(np.mean(block_p99s))}")
            print()
...
def main():
    parser = argparse.ArgumentParser(description='Generate throughput statistics for experiment runs')
    parser.add_argument('data_dirs', nargs='+', help='Data directory(ies) (BLOCK-PROMO-2025-08-31, ZONED-PROMO-2025-08-30, etc.)')
    parser.add_argument('--metric', '-m', default='bytes_total',
                       help='Metric to analyze (default: bytes_total)')
    ...
    generate_throughput_statistics(args.data_dirs, args.metric, args.bucket_seconds, args.compare, args.latex)
